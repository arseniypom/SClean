...
//  SClean
//
//  Displays a grid of photos for a specific month
//
...
struct MonthGridView: View {
    ...
    init(bucket: MonthBucket, permissionService: PhotoPermissionService) {
        self.bucket = bucket
        self.permissionService = permissionService
        self._photosService = StateObject(wrappedValue: MonthPhotosService(year: bucket.year, month: bucket.month))
    }
        ...
        if !permissionService.status.canAccessPhotos {
            ...
            case .empty:
                EmptyStateView(
                    ...
                    title: "No Photos",
                    message: "No photos found for \(bucket.displayName)."
                )
...
#Preview {
    let bucket = MonthBucket(id: "2024-03", year: 2024, month: 3, count: 100, totalBytes: 500_000_000)
    let permissionService = PhotoPermissionService()
    return NavigationStack {
        MonthGridView(bucket: bucket, permissionService: permissionService)
    }
}
